...
- Layout: studio/open-plan and/or one-, two-, three-, or four-bedroom units. Normalize natural input such as `一房`, `两房`, `兩房`, `二房`, `三房`, `四房`, `2 bedroom`, and `three bedrooms` to canonical scanner terms such as `1房`, `2房`, `3房`, and `4房` before saving the task or pushing API filters.
...
- Budget stats using rent bands and low-rent signals derived dynamically from the whole local listing database's rent distribution, not fixed hard-coded budget thresholds.
