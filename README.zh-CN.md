...
**免责声明：** 本项目与任何地产代理公司均无从属、授权、背书或合作关系，纯属第三方工具。使用者须自行确认其使用方式符合相关网站的使用条款、授权范围及适用法律。
...
本系统仅供个人进行本地市场观察与分析之用。使用时应控制请求频率、保留来源链接，并严格遵守各网站的服务条款。请勿将 `first_delisted_at` 视为真实成交时间。系统默认不采集、不存储经纪人姓名及电话等个人资料，亦不持久化来源 API 或网页的原始数据（Payload）。
